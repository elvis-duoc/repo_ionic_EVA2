...
export class ProfilePage implements AfterViewInit, OnDestroy, OnInit {
  user: ProfileModel;
  ...
  async showAlertError() {  
    const alert = await this.alertCtrl.create({  
      header: '¡ERROR - Código ya existente!',  
      subHeader: '',  
      message: 'Este código ya fue registrado, intente otro',  
      buttons: ['Cerrar']  
    });  
    await alert.present();  
  ...
  async showAlert() {  
    const alert = await this.alertCtrl.create({  
      ...
      subHeader: '',  
      message: 'Tu asistencia se registró con éxito',  
      buttons: ['Aceptar']  
    });  
    await alert.present();  
    const result = await alert.onDidDismiss();  
